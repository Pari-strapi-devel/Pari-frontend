...
const ContactForm = () => {
  ...
  // Get background image from current article or use fallback
  const backgroundImage = articles.length > 0
    ? formatArticleData(articles[currentImageIndex]).imageUrl
    : '/images/banners/library-banner.jpg';
  ...
  return (
    <div
      className="md:min-h-screen mx-auto bg-cover bg-center bg-no-repeat relative transition-all duration-1000 ease-in-out dark:bg-background"
      style={{
        backgroundImage: `linear-gradient(rgba(0, 0, 0, ${overlayOpacity}), rgba(0, 0, 0, ${overlayOpacity})), url('${backgroundImage}')`
      }}
    >
      <div className="max-w-[1232px] mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-1 lg:gap-12 items-center min-h-[calc(100vh-10rem)] lg:min-h-[calc(100vh-8rem)]">
          ...
          <div className="bg-popover p-6 sm:p-8 rounded-[16px] w-full max-w-md mx-auto" style={{ boxShadow: '0px 2px 2px 0px #0000001A', backdropFilter: 'blur(0px)' }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <button
                type="submit"
                className="w-full bg-primary-PARI-Red dark:bg-primary-PARI-Red text-white py-2.5 sm:py-3 px-4 sm:px-6 rounded-md font-medium hover:bg-red-700 dark:hover:bg-red-700 transition-colors duration-200 text-sm sm:text-base"
              >
                Send message
              </button>
            </form>
